...
pub trait Num: Clone + Copy + Debug + Eq + Ord + PartialEq + PartialOrd + Sized {
    ...
    where
        Self::Raw: raw::Add<Other::Raw, Output::Raw>,
    {
        const {
        ...
        unsafe { Output::new_unchecked(raw::Add::add(self.raw(), other.raw())) }
    }
    fn sub<Other: Num, Output: Num>(self, other: Other) -> Output
    where
        Self::Raw: raw::Sub<Other::Raw, Output::Raw>,
    {
        const {
            assert!(Output::SHIFT == Self::SHIFT);
            assert!(Output::SHIFT == Other::SHIFT);
        ...
        unsafe { Output::new_unchecked(raw::Sub::sub(self.raw(), other.raw())) }
    }
    fn neg<Output: Num>(self) -> Output
    where
        Self::Raw: raw::Neg<Output::Raw>,
    {
        unsafe { Output::new_unchecked(raw::Neg::neg(self.raw())) }
    }
    fn mul<Other: Num, Output: Num>(self, other: Other) -> Output
    where
        Self::Raw: raw::Mul<Other::Raw, Output::Raw>,
    {
        const {
            assert!(Output::SHIFT == Self::SHIFT + Other::SHIFT);
            assert!(Output::BITS >= Self::BITS + Other::BITS);
        }
        unsafe { Output::new_unchecked(raw::Mul::mul(self.raw(), other.raw())) }
    }
    fn div<Other: Num, Output: Num>(self, other: Other) -> Output
    where
        Self::Raw: raw::Div<Other::Raw, Output::Raw>,
    {
        const {
            assert!(Output::SHIFT == Self::SHIFT - Other::SHIFT);
            assert!(Output::BITS > Self::BITS);
        }
        unsafe { Output::new_unchecked(raw::Div::div(self.raw(), other.raw())) }
    }
}
